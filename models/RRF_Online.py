...
class RRF_Online(Module):
    ...
    def _init_param(self, gamma, w, loss_type):
        ...
        #     self.gamma = Tensor_type(np.log(gamma) * np.ones((self.num_feature, 1)))  # log 형태로 save
        # else:
        #     self.gamma = Tensor_type(np.log(gamma))
            # weight
        if w is None:
    ...
    def online_learning(self, logger):
        start = time.time()
        logger.info("==" * 20)
        logger.info(self.model_name + '_' + str(self.gamma.exp()) + '_' + str(self.lr_RRF_gamma) + '_' + str(self.lr_RRF_w) + '_' + str(
        ...
        for t in range(self.X.shape[0]):
            ...
            if t % 1000 == 0:
                ...
                logger.info(' %d th : pred %f , real %f ' % (t, scalar, self.Y[t],))
...
if __name__ == "__main__":
    data_dir = os.pardir + '/data/ml-100k/'
